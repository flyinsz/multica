...
func (c crmPendingReplyCandidate) missingDraftContextReasons() []string {
	...
	if c.AccountID.Valid && !c.HasAccountProfile {
		reasons = append(reasons, "客户档案为空或未生成")
	}
	return reasons
}
...
func (h *Handler) runCRMPendingReplyAutomationForThreads(ctx context.Context, workspaceID pgtype.UUID, threadIDs []pgtype.UUID, limit int, config json.RawMessage) (map[string]any, error) {
	...
	rows, err := h.DB.Query(ctx, `
		WITH latest AS (
			SELECT DISTINCT ON (m.thread_id)
				m.thread_id, m.id AS message_id, m.direction, m.subject, m.from_email, m.received_at, m.sent_at, m.created_at, m.is_read, m.is_trashed, m.folder, m.body_text, m.body_html, m.snippet
		...
		SELECT t.id,
		       ...
		       COALESCE(latest.snippet, ''),
		       EXISTS (SELECT 1 FROM crm_account_profile p WHERE p.workspace_id=$1 AND p.account_id=t.account_id AND (COALESCE(p.summary,'')<>'' OR p.profile_json <> '{}'::jsonb))
	...
	for rows.Next() {
		...
		if err := rows.Scan(&candidate.ThreadID, &candidate.MessageID, &candidate.AccountID, &candidate.ContactID, &candidate.AccountName, &candidate.OwnerType, &candidate.OwnerMemberID, &candidate.OwnerAgentID, &candidate.Subject, &candidate.LatestAt, &candidate.BodyText, &candidate.BodyHTML, &candidate.Snippet, &candidate.HasAccountProfile); err != nil {
			continue
		}
		candidates++
		...
		projectID, err := h.ensureCRMAccountProject(ctx, workspaceID, candidate.AccountID, candidate.AccountName)
		if err != nil {
...
func (h *Handler) runCRMDueFollowupAutomation(ctx context.Context, workspaceID pgtype.UUID, limit int, config json.RawMessage) (map[string]any, error) {
	...
	for rows.Next() {
		...
		defaultBody := fmt.Sprintf("CRM 到期客户跟进自动创建。\n客户：%s\n客户ID：%s\n到期时间：%s\n\nAI上下文摘要（profile + 近期互动，不含全量历史）：\n%s\n\n处理要求：\n1. Issue 初始负责人使用 CRM AI 配置中的 todo 阶段负责人，由其生成客户跟进邮件草稿。\n2. 先检查近期邮件/未来 WhatsApp 互动，确认是否仍需跟进；若客户已回复或到期日已过期处理，请更新 next_follow_up_at 或关闭该 Issue。\n3. 草稿生成完成后，必须把 Issue 从 todo 转入审核阶段，并把负责人改为邮件草稿审核人。\n4. %s\n5. 审核通过后才能发送邮件。%s", name, uuidToString(accountID), timestampToString(due), contextSummary, reviewerLine, lowConfidenceNote)
		body := defaultBody
		if strings.TrimSpace(issueActors.IssueTemplate) != "" {
			body = h.renderCRMAIIssueTemplate(issueActors.IssueTemplate, map[string]string{
				"account_name":    name,
				"account_id":      uuidToString(accountID),
				"latest_at":       timestampToString(due),
...
func (h *Handler) crmDraftReviewerLine(ctx context.Context, workspaceID pgtype.UUID, ownerType string, reviewerMemberID, reviewerAgentID pgtype.UUID) string {
	if ownerType == "agent" && reviewerAgentID.Valid {
		var name string
		if err := h.DB.QueryRow(ctx, `SELECT COALESCE(name,'') FROM agent WHERE workspace_id=$1 AND id=$2 AND archived_at IS NULL LIMIT 1`, workspaceID, reviewerAgentID).Scan(&name); err == nil {
...
func (h *Handler) buildCRMPendingReplyIssueBody(candidate crmPendingReplyCandidate, messageLink, latestAt, reviewerLine string, missingReasons []string, aiContext crmEmailAIContext, cfg crmAIIssueActorConfig) string {
	...
	defaultInstructions := fmt.Sprintf("任务：CRM Pending Reply 小队协作。\n\n规则：每条评论只触发下一步一个执行者，必须用 exact mention markdown，不要 plain @Name，不要在同一条分工评论里同时 @draft agent 和 @reviewer。\n\n如果本 issue 分配给小队：第一步由 Master 做分工。上下文足够时只 @默认 draft agent 创建草稿；上下文不足时只 @Researcher 补充。Draft agent 创建/修改草稿后只贴 draft_mention（形如 [Draft <id>](mention://crm-draft/<id>)），不要贴长 draft_url；只 @Master 复核。Master 复核通过后，再单独 @动态 reviewer 审核。\n\n如果 reviewer 是 agent：审核无误后通过 CRM MCP 发送邮件草稿，并把 issue 状态改为 done；不通过则只 @默认 draft agent 修改现有草稿。\n如果 reviewer 是 member：member 确认无误后只 @mention Master 发送邮件；Master 发送邮件后把 issue 状态改为 done。\n如果本 issue 直接分配给 agent：按该 agent 角色处理。\n\n默认 draft agent：%s\nReviewer：%s\n\n必须：\n- 通过 CRM MCP 读取当前邮件线程和客户 profile；UUID 参数只用纯 UUID。\n- 缺上下文不是“不回复”的理由。\n- 草稿正文语言：%s。\n- 草稿正文格式：%s。", draftAgentLine, reviewerLine, draftLanguage, replyFormat)
...
func (h *Handler) buildCRMPendingReplyMergeComment(candidate crmPendingReplyCandidate, messageLink, latestAt, reviewerLine string, missingReasons []string) string {
	contextLine := "上下文足够，可继续草稿生成或更新现有草稿。"
	if len(missingReasons) > 0 {
...
func (h *Handler) createCRMPendingReplyDraft(ctx context.Context, workspaceID, issueID pgtype.UUID, candidate crmPendingReplyCandidate, reviewerLine string, aiContext crmEmailAIContext, cfg crmAIIssueActorConfig) error {
	...
	if strings.TrimSpace(fromEmail) == "" {
		return h.addCRMInternalIssueComment(ctx, workspaceID, issueID, "未生成待审核回复草稿：最新邮件缺少发件人地址。")
	}
...
func (h *Handler) createCRMAIFollowupDraft(ctx context.Context, workspaceID, issueID, accountID pgtype.UUID, accountName, dueText string) error {
	...
	aiContext := h.buildCRMEmailAIContext(ctx, workspaceID, accountID, contactID, pgtype.UUID{}, "crm_due_followup_draft", 6)
	wikiContext := strings.TrimSpace(aiContext.String())
	if wikiContext == "" {
		wikiContext = "Customer Wiki 暂无可用内容。"
	}
	reason := fmt.Sprintf("草稿思路：这是到期跟进自动化生成的跟进草稿。已优先读取 Customer Wiki / 客户 Profile。依据客户 next_follow_up_at 到期时间 %s、客户画像、近期互动和主联系人生成，目标是礼貌重启沟通并确认下一步需求。风险：发送前请人工确认报价、交期、项目上下文和联系人是否正确。\n\nCustomer Wiki 摘要：\n%s", dueText, wikiContext)
	body := fmt.Sprintf("Hi %s,\n\nI hope you are doing well. I wanted to follow up with you regarding %s and check whether there are any updates or new requirements we can support.", contactName, accountName)
	var draftID pgtype.UUID
...
func (h *Handler) crmAIIssueActors(ctx context.Context, workspaceID pgtype.UUID, config json.RawMessage) (crmAIIssueActorConfig, error) {
	...
	if cfg.ResearcherType == "" || cfg.ResearcherID == "" {
		if agentID, err := h.crmAgentByName(ctx, workspaceID, "Researcher"); err == nil && agentID.Valid {
			cfg.ResearcherType = "agent"
	...
	if cfg.ResearcherType != "" || cfg.ResearcherID != "" {
		if err := h.validateCRMIssueActor(ctx, workspaceID, cfg.ResearcherType, cfg.ResearcherID); err != nil {
			return cfg, fmt.Errorf("invalid CRM AI issue researcher assignee: %w", err)
		}
	}
	return cfg, nil
}
