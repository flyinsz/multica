...
export function CRMEmailsPage() {
  ...
  const selectedIssueSummaries: Array<{ id: string; number?: number; identifier?: string; title?: string; status?: string }> = selectedThread?.issues?.length ? selectedThread.issues : selectedIssueIds.map((id) => issues.find((issue) => issue.id === id)).filter(Boolean).map((issue) => ({ id: issue!.id, identifier: issue!.identifier, title: issue!.title, status: issue!.status }));
