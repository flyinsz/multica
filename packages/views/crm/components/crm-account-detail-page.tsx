        ...
        <div className="min-h-0 min-w-0 flex-1 overflow-x-hidden overflow-y-auto p-6">
          ...
          <TabsContent value="notes" className="min-w-0 space-y-6">
            <section className="min-w-0 overflow-hidden rounded-lg border bg-card">
              ...
              {notesLoading ? <div className="space-y-2 p-4"><Skeleton className="h-16 w-full" /><Skeleton className="h-16 w-full" /></div> : notes.length === 0 ? <div className="p-8 text-center text-sm text-muted-foreground">{t(($) => $.notes.empty)}</div> : <div className="min-w-0 divide-y overflow-hidden">{notes.map((note) => <div key={note.id} className="min-w-0 max-w-full overflow-hidden px-4 py-3 text-sm"><div className="min-w-0 break-all font-medium">{note.subject || t(($) => $.notes.untitled)}</div><div className="mt-1 min-w-0 whitespace-pre-wrap break-all">{note.body}</div><div className="mt-2 min-w-0 break-all text-xs text-muted-foreground"><ChannelLabel channel={note.channel} t={t} /> · {t(($) => $.directions[note.direction])} · {new Date(note.occurred_at).toLocaleString()}</div></div>)}</div>}
            </section>
          </TabsContent>
        </div>
      </Tabs>
