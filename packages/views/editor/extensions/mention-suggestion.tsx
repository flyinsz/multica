...
export interface MentionItem {
  id: string;
  label: string;
  type: "member" | "agent" | "squad" | "issue" | "project" | "all" | "crm-account" | "crm-contact";
...
export const MentionList = forwardRef<MentionListRef, MentionListProps>(
  function MentionList({ items, query, command, includeProjectSearch = false }, ref) {
        ...
        void (async () => {
          try {
            ...
            if (includeProjectSearch && q) {
              searches.push(
                api.searchProjects({
                  ...
                  res.projects.map((project) => ({ ...projectToMention(project), group: "search" as const })),
                ),
              );
            }
            ...
            if (!cancelled && !controller.signal.aborted) {
              setServerItems(results.flatMap((result) => result.status === "fulfilled" ? result.value : []));
            }
          } catch {
    ...
    const displayItems = useMemo(() => {
      ...
      return rankMentionItems(mergeMentionItems(items, currentServerItems)).slice(0, MAX_ITEMS);
    ...
    // One scroll container for every group. The context variant only differs in width / max-height / chrome.
